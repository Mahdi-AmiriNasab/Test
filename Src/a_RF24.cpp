...
uint8_t RF24::read_register(uint8_t reg, uint8_t* buf, uint8_t len)
{
  ...
  *ptx++ = ( R_REGISTER | ( REGISTER_MASK & reg ) );
  ...
  endTransaction();
	...
	NRF24L01_CSN_LOW;
	HAL_Delay(1);
	status = HAL_SPI_Transmit(&NRF24L01_SPI,(uint8_t *) ( R_REGISTER | ( REGISTER_MASK & reg )), 1, 100); 
	HAL_Delay(1);
	HAL_SPI_Receive(&NRF24L01_SPI, buf, len, 1000);		
	HAL_Delay(1);
	NRF24L01_CSN_HIGH;
#endif
...
uint8_t RF24::write_register(uint8_t reg, const uint8_t* buf, uint8_t len)
{
  ...
  *ptx++ = ( W_REGISTER | ( REGISTER_MASK & reg ) );
  ...
  endTransaction();
	...
	HAL_Delay(1);
	HAL_SPI_Transmit(&NRF24L01_SPI,(uint8_t *)buf , len, 1000);		
	HAL_Delay(1);
	NRF24L01_CSN_HIGH;
	#endif
...
uint8_t RF24::write_register(uint8_t reg, uint8_t value)
{
  uint8_t status;
	#if CDC_LOG
	uint8_t bf[100];
	sprintf((char *)bf,"write_register(%02x,%02x)\r\n", reg, value);
...
uint8_t RF24::write_payload(const void* buf, uint8_t data_len, const uint8_t writeType)
{
  ...
  #if CDC_LOG
	uint8_t bf[100];
	sprintf((char *)bf,"[Writing %u bytes %u blanks]\n",data_len ,blank_len);
	CDC_Transmit_FS(bf, strlen((char *)bf));
	#endif
 #if defined (RF24_LINUX)
	beginTransaction();
  ...
  endTransaction();
	...
	HAL_Delay(1);
	HAL_SPI_Transmit(&NRF24L01_SPI, 0, blank_len, 1000);		
	HAL_Delay(1);
	NRF24L01_CSN_HIGH;
	#endif
...
uint8_t RF24::read_payload(void* buf, uint8_t data_len)
{
  ...
  #if CDC_LOG
	uint8_t bf[100];
	sprintf((char *)bf,"[Reading %u bytes %u blanks]\n",data_len,blank_len);
	CDC_Transmit_FS(bf, strlen((char *)bf));
	#endif
  #if defined (RF24_LINUX)
	beginTransaction();
  ...
  endTransaction();
	...
	NRF24L01_CSN_LOW;
	HAL_Delay(1);
	status = HAL_SPI_Transmit(&NRF24L01_SPI, (uint8_t *)R_RX_PAYLOAD, 1, 100); 
	HAL_Delay(1);
	HAL_SPI_Receive(&NRF24L01_SPI, (uint8_t *)buf, data_len, 1000);		
	HAL_Delay(1);
	HAL_SPI_Transmit(&NRF24L01_SPI, (uint8_t *)0Xff, blank_len, 1000); 	//to read the blank payload
	HAL_Delay(1);
	NRF24L01_CSN_HIGH;
	#endif
...
{
  NRF24L01_CE_LOW; // Guarantee CE is low on powerDown
